...
class Stack_joint_position_dataset(Dataset):  
    ...
    def stack_joint_position(self, key, index):
        ...
        for ii in range(self.nb_per_stack):
            ...
            x0,y0,x1,y1=self.gt_bounding_box(key,index+ii)
            data = self.crop_and_resize(mat,x0,y0,x1,y1)
            out[ii,:,:] = data
    
        return torch.from_numpy(out).float().div(255)

    def gt_bounding_box(self,videoname,idx):
        annotation = scipy.io.loadmat(self.anno_path+videoname+'.mat')
        x0,y0,x1,y1 = annotation['bbox'][idx-1]

        return int(x0),int(y0),int(x1),int(y1)

    def crop_and_resize(self,mat,x0,y0,x1,y1):
        joint_postion = Image.fromarray(mat.sum(axis=2,dtype='uint8'))
        crop_img = joint_postion.crop([x0,y0,x1,y1])
        crop_size = crop_img.size
        if (crop_size[0] >= crop_size[1]):
            base_size = (crop_size[0],crop_size[0])
        else:
            base_size = (crop_size[1],crop_size[1])
        
        base_img = Image.new('L', base_size)
        base_img.paste(crop_img, ((base_size[0] - crop_size[0]) / 2, (base_size[1] - crop_size[1]) / 2))

        return base_img.resize([224,224])
