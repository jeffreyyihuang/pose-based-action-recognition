...
class stack_opf_dataset(Dataset):  
    ...
    def stack_opf(self, key, index):
        ...
        annotation = scipy.io.loadmat(self.anno_path+key+'.mat')
        x0,y0,x1,y1 = annotation['bbox'][index-1]
        out=np.zeros((2*self.nb_per_stack,224,224))

        for ii in range(self.nb_per_stack):
            flowx=Image.open(self.root_dir + key+'/x'+str(index+ii).zfill(6)+'.jpg')
            flowy=Image.open(self.root_dir + key+'/y'+str(index+ii).zfill(6)+'.jpg')

            cropx = flowx.crop([x0,y0,x1,y1])
            cropy = flowy.crop([x0,y0,x1,y1])

            out[2*(ii),:,:] = self.transform(cropx)
            out[2*(ii)+1,:,:] = self.transform(cropy)
...
if __name__ == '__main__':
    data_loader = Stack_opf_DataLoader(BATCH_SIZE=1,num_workers=1,nb_per_stack=10,
                                        ...
                                        data_path='/home/ubuntu/data/PennAction/Penn_Action/flownet2.0/dense_opf/',
